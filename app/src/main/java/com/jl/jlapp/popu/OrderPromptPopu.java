...
public class OrderPromptPopu extends PopupWindow {
    ...
    public OrderPromptPopu(Context context, View.OnClickListener itemclick, Integer isFromWhichPage, Integer stock) {
        super(context);

        LayoutInflater layoutInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        view = layoutInflater.inflate(R.layout.order_prompt_popu, null);
        ...
        if (isFromWhichPage == 1) {
            popuContent.setText("抱歉，您购买的商品均已售空，请选择其他同类商品。");
            cancelBtn.setVisibility(View.GONE);
            okBtn.setText("返回首页");
        } else if (isFromWhichPage == 2) {
            popuContent.setText("抱歉，您购买的部分商品已售空，继续下单则移出已售空商品。");
        } else if (isFromWhichPage == 3) {
            popuContent.setText("抱歉，您购买的部分商品已售空或库存不足，继续下单则移出已售空商品以及购买部分商品的剩余数量。");
        } else if (isFromWhichPage == 4) {
            popuContent.setText("抱歉，您选购的商品仅剩" + stock + "件，继续下单将购买当前商品的剩余数量。");
        } else if (isFromWhichPage == 5) {
            popuContent.setText(" 抱歉，您购买的部分商品库存数量不足，继续下单将购买当前商品的剩余数量。");
        }
